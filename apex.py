...
def admin():
    ...
    participant_list_html = ''.join(f'''<li>{i+1}. {name}<form method="post" action="/remove" style="display:inline;"><input type="hidden" name="name" value="{name}"><button type="submit">削除</button></form></li>''' for i, name in enumerate(participants))

    candidate_list_html = ''.join(f'''<li>{name}<form method="post" action="/add_from_candidate" style="display:inline;"><input type="hidden" name="name" value="{name}"><button type="submit">参加に追加</button></form></li>''' for name in sorted(candidates - set(participants)))
